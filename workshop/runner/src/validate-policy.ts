...
export async function validateParticipantPolicy(requireChange: boolean): Promise<void> {
	const current = splitPolicy(await readFile(resolve("policies/h4.md"), "utf8"));
	const starter = splitPolicy(await readFile(resolve("policies/h4.default.md"), "utf8"));
	if (current[0] !== starter[0] || current[2].trimEnd() !== starter[2].trimEnd()) throw new Error("Only text inside the H4 participant edit boundary may change. Run npm run reset:h4 to recover.");
...
if (process.argv[1]?.endsWith("validate-policy.ts")) void validateParticipantPolicy(process.argv.includes("--require-change")).then(() => console.log("H4 policy boundary valid"));
